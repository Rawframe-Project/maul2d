...
{
    m2Body_ApplyLinearImpulseAtPoint(body, (m2Vec2){impulseX, 0.0f}, m2Body_GetPosition(body));
}

int main(void)
{
    // The journal embeds a full snapshot up front, so size the tape
    // from m2World_GetJournalBaseSize plus room for the ops you expect.
    ...
    int tapeCapacity = m2World_GetJournalBaseSize(world) + (1 << 16);
